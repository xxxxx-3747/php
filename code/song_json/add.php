...
function add() {
  ...
  if(empty($_POST['title'])){
    $GLOBALS['error'] = '请输入标题名称';
    return;
  }
  //判断歌手是否为空
  if(empty($_POST['artist'])){
    $GLOBALS['error'] = '请输入歌手名';
    return;
  }
  //判断海报域是否为空
  if(empty($_FILES['images'])){
    $GLOBALS['error'] = '请上传海报文件1';
    return;
  }
  ...
  if($file_images['error'] !== UPLOAD_ERR_OK ){
    $GLOBALS['error'] = '请上传海报文件2';
    return;
  }
  //判断海报的大小是否合适
  if($file_images['size'] < 1 * 1024 *1024 ){
    $GLOBALS['error'] = '上传的海报文件太小';
    return;
  }
  if($file_images['size'] > 10 * 1024 *1024 ){
    $GLOBALS['error'] = '上传的海报文件太大';
    return;
  }
  //判断海报的文件格式是否合法
